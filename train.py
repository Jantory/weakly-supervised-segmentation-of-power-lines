...
def main():
    ...
    netG_A2B.load_state_dict(T.load('wire_remove_model_weights.pth'))
    netG_B2A.load_state_dict(T.load('wire_add_model_weights.pth'))
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        tsboard.add_scalar("Generator/Total", np.mean(losses["loss_G"]), epoch)
        tsboard.add_scalar("Generator/loss_identity_A", np.mean(losses["loss_identity_A"]), epoch)
        tsboard.add_scalar("Generator/loss_identity_B", np.mean(losses["loss_identity_B"]), epoch)
        tsboard.add_scalar("Generator/loss_GAN_A2B", np.mean(losses["loss_GAN_A2B"]), epoch)
        tsboard.add_scalar("Generator/loss_GAN_B2A", np.mean(losses["loss_GAN_B2A"]), epoch)
        tsboard.add_scalar("Generator/loss_cycle_ABA", np.mean(losses["loss_cycle_ABA"]), epoch)
        tsboard.add_scalar("Generator/loss_cycle_BAB", np.mean(losses["loss_cycle_BAB"]), epoch)

        tsboard.add_scalar("Discriminator/loss_D_A", np.mean(losses["loss_D_A"]), epoch)
        tsboard.add_scalar("Discriminator/loss_D_B", np.mean(losses["loss_D_B"]), epoch)
