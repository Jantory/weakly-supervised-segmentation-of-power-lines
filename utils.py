...
def mask_generator(shadow, shadow_free):
    ...
    for i in range(shadow.shape[0]):
        ...
        im_s = to_gray(to_pil(((shadow[i,:].data.squeeze(0) + 1.0) * 0.5).cpu()))
        ...
        masks.append((np.float32(diff >= L)-0.5)/0.5)

    mask = torch.tensor(np.stack(masks, axis = 0)).unsqueeze(1).cuda() #-1.0:non-shadow, 1.0:shadow
